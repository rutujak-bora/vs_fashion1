...
export default function ShippingPolicy() {
  ...
  const fetchContent = async () => {
    try {
      const response = await axios.get(`${API}/content/shipping`);
      setContent(response.data.content || 'Shipping Policy content will be available soon.');
    } catch (error) {
      console.error('Error fetching content:', error);
      setContent('Shipping Policy content will be available soon.');
    } finally {
      setLoading(false);
    }
  };
